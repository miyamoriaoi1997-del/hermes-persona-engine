...
class AIAgent:
    ...
    def __init__(
        ...
        # Initialize emotion system for Rio persona
        self.emotion_manager = None
        self.auto_emotion_update = True
        try:
            ...
            logger.debug("Emotion system initialization failed: %s", e)
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Inject emotion-based tone modifiers after SOUL.md
        if self.emotion_manager:
            try:
                tone_modifiers = self.emotion_manager.get_tone_modifiers()
                if tone_modifiers:
                    prompt_parts.append(tone_modifiers)
            except Exception as e:
                logger.debug("Failed to get emotion tone modifiers: %s", e)
    ...
    def run_conversation(
        ...
        # Apply time decay to emotion state at conversation start (Rio persona)
        if self.emotion_manager:
            try:
                self.emotion_manager.apply_time_decay_if_needed()
            except Exception as e:
                logger.debug("Time decay application failed: %s", e)
        
        # Update emotion state BEFORE generating response (Rio persona - real-time reaction)
        # User's message affects current reply, not next reply
        if self.emotion_manager and self.auto_emotion_update:
            try:
                # Build messages list with current user message for emotion detection
                _temp_messages = list(conversation_history) if conversation_history else []
                _temp_messages.append({"role": "user", "content": user_message})
                
                # Detect emotion event
                event = self.emotion_manager.detector.detect_emotion_event(_temp_messages)
                
                # Update emotion state
                self.emotion_manager.update_emotion_state(_temp_messages)
                ...
                if event and event.trigger_type in {"intimacy", "praise", "criticism", "care", "milestone"}:
                    try:
                        ...
                        context = user_text[:100] if len(user_text) > 100 else user_text
                        ...
                        logger.debug("Moment recording failed: %s", e)
            except Exception as e:
                logger.debug("Pre-response emotion update failed: %s", e)
        ...
        # Emotion state already updated at conversation start (Rio persona)
        # No need to update again at conversation end to avoid duplicate processing
